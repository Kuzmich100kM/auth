...
module.exports.registration = async (req, res, next) => {
	try {
		...
		const { _id: userId, role, isActivated } = user
		...
		return res.status(200).json({ userId, email, role, isActivated, accessToken })
	} catch (err) {
		next(err)
	}
}
...
module.exports.login = async (req, res, next) => {
	try {
		...
		const { _id: userId, password: hashedPass, role, isActivated } = user
...
module.exports.refresh = async (req, res, next) => {
	try {
		...
		const { _id: userId, email, role, isActivated } = tokenFromDb
